...
# --- Resilience Fix: Force Load API Key from Secrets ---
# Sometimes modifying Config directly in a module isn't enough depending on import order.
# We do this at the app level to be safe.
try:
    if not Config.GOOGLE_API_KEY and "GOOGLE_API_KEY" in st.secrets:
        # Detected in secrets but not in Config (maybe Config loaded before secrets were ready)
...
try:
    ...
    st.error("If this is about GOOGLE_API_KEY, please ensure it is set in .env or Streamlit Secrets.")
    st.info("You need a Google Gemini API Key to run this app.")
    st.stop()
